function writeMesh(data,file_name)


gmsh = fopen(file_name , 'w','n');
assert(gmsh>=0,'WriteGMSH', 'Error abriendo fichero'); %comprobaci�n de que fopen ha funcionado
...
%la instrucci�n switch hace que seg�n el valor de n_dim ejecute unas
%instrucciones u otras

fprintf(gmsh, '$MeshFormat\n'); %pasar a l�nea siguiente se hace con \n
fprintf(gmsh, '2.2 0 8\n');
fprintf(gmsh, '$EndMeshFormat\n');

fprintf(gmsh, '$Nodes\n');
fprintf(gmsh, '%d \n', nn); %introducimos un car�cter de sustituci�n %d para reescribir el contenido de una variable
coords = zeros(1,3);
for i = 1:nn
...
n_tags=2; tag1=0; %la tag 1 es un 0 y la tag 2 es el n�mero de elementos
...
for e = 1:ne
    ...
    for ne = 1:nne
        ...
        %pues en principio no sabemos cu�ntos nodos va a tener nuestro
        %elemento
    end
      fprintf(gmsh, ' \n');
end
